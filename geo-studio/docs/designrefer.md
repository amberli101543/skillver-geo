...
## GEO Studio Implementation (SPEC-GEO-033)

**Token source file:** `web/src/design-tokens.css` (imported by `web/src/styles.css`)

### CSS class mapping

| designrefer component | GEO Studio class | Legacy alias |
|---|---|---|
| `button-primary` | `.ds-button-primary` | `.btn-accent` |
| `button-secondary` | `.ds-button-secondary` | `.btn-secondary` |
| `card-base` | `.ds-card-base` | `.card` |
| `text-input` | `.ds-text-input` | form `input` / `select` / `textarea` |
| `badge-green-soft` | `.ds-badge-green-soft` | `.badge-ok` |
| `hero-band-dark` | `.ds-hero-band-dark` | `.dashboard-header`, `.login-hero` |
| `pill-tab` | `.ds-pill-tab` | `.header-nav-btn` |

### Typography utilities

| designrefer token | CSS class |
|---|---|
...
All color, spacing, radius, and elevation tokens are exposed as `--color-*`, `--space-*`, `--radius-*`, and `--elev-*` custom properties with legacy `--brand-green`, `--hairline`, etc. aliases for existing styles.

### Dashboard responsive (SPEC-GEO-034)

| Breakpoint | Behavior |
|---|---|
| `< 1024px` | Header nav collapses; **模块导航** toggle reveals full-width pill links (44px touch targets) |
| `640–1023px` | Tablet padding; charts 2-column; brand actions wrap |
| `< 640px` | Single-column forms/tables (`.responsive-table` card layout); stacked brand actions |
