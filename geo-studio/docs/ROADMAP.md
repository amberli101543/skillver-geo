...
> **统筹入口已迁移**：v1 之后的开发计划、阶段与进度请读 **[DEVELOPMENT-PLAN.md](./DEVELOPMENT-PLAN.md)** 与 **[PROGRESS.md](./PROGRESS.md)**。  
> 机器可读计划：[`cursor-kit/plan/master-plan.yaml`](../cursor-kit/plan/master-plan.yaml)  
> 本文件保留 v1 历史、约束与 DoD 摘要。
...
新需求一律进 **[DEVELOPMENT-PLAN.md](./DEVELOPMENT-PLAN.md)** Phase 2~4 backlog；需明确批准并将 SPEC 设为 `ACTIVE` 后才可开工。

## v2 Backlog（摘要 — 完整见 DEVELOPMENT-PLAN）

| Phase | SPEC | 主题 |
|-------|------|------|
| 1 架构 | ARCH-001~004 | Ai Facade、Job 异步、RAG、连接器注册表 |
| 2 AI | GEO-027~029 | 多 LLM、真实引擎、Prompt UI |
| 3 平台 | GEO-030~032 | BullMQ、告警外推、独立部署 |
| 4 体验 | GEO-033~034 | 设计系统、响应式 |

**不做**：RBAC / 多租户（与单用户前提冲突）。

## 产品 v1 Definition of Done（摘要）

完整条文见 [`cursor-kit/specs/SPEC-GEO-PROD-001.yaml`](../cursor-kit/specs/SPEC-GEO-PROD-001.yaml)。
...
> 已迁移至 [DEVELOPMENT-PLAN.md](./DEVELOPMENT-PLAN.md)。下表仅作快速索引。

| 项 | SPEC |
|----|------|
| 架构治理 | ARCH-001 ~ 004 |
| 多 LLM / 多引擎 | GEO-027, GEO-028 |
| Prompt UI | GEO-029 |
| 任务队列 | GEO-030 |
| 告警外推 | GEO-031 |
| ai / worker 独立部署 | GEO-032 |
| 高保真 UI / Figma | GEO-033 |
| 移动端响应式 | GEO-034 |
| RBAC / 多租户 | ➖ 不做 |
