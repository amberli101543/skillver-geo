...
- [ ] 【人工】W1 信任先行：C13「会不会挂我」、C16「封闭内测真实性」等 G 系列稿（发布前确认邀请码准入，并复核费用/次数/导师状态等待核实声明）
...
1. **口径检查**：`01-strategy/` 与 `04-channels/` 的封闭内测/邀请码准入表述应保留；每篇发布前须清除开放准入叙事，并人工核实费用、具体体验或准入数量、导师服务状态。
