...
- [ ] 【人工】W1 信任先行：C13「会不会挂我」、C16「开放注册与社群邀请码」等 G 系列稿（发布前确认开放注册口径，并复核费用/次数/导师状态等待核实声明）
...
1. **口径检查**：`01-strategy/` 与 `04-channels/` 须同步「全面开放注册；邀请码仅用于社群参与」；每篇发布前清除「需邀请码才能注册 / 封闭内测准入」旧叙事，并人工核实费用、具体体验或名额数量、导师服务状态。
